// ============================================================================
// File: ascon128_d1.v
// Description: Masked Ascon-128 AEAD Core for Order D = 1 (3 shares)
// ============================================================================
...
module ascon128_d1 #(
    ...
    // 3 Shares of Key, Nonce, AD, Data In
    input  wire [k-1:0] key_0, key_1, key_2,
    input  wire [127:0] nonce_0, nonce_1, nonce_2,
    input  wire [l-1:0] ad_0, ad_1, ad_2,
    input  wire [y-1:0] data_in_0, data_in_1, data_in_2,
    ...
    // Outputs
    output wire [y-1:0] data_out_0, data_out_1, data_out_2,
    output wire [y-1:0] data_out,
    output wire [127:0] tag_out_0, tag_out_1, tag_out_2,
    ...
    // Output Registers
    reg [Y-1:0] out_buf_0, out_buf_1, out_buf_2;
    reg [127:0] tag_reg_0, tag_reg_1, tag_reg_2;

    assign data_out_0 = out_buf_0[Y-1 -: y];
    assign data_out_1 = out_buf_1[Y-1 -: y];
    assign data_out_2 = out_buf_2[Y-1 -: y];
    assign data_out   = data_out_0 ^ data_out_1 ^ data_out_2;

    assign tag_out_0  = tag_reg_0;
    assign tag_out_1  = tag_reg_1;
    assign tag_out_2  = tag_reg_2;
    assign tag_out    = tag_reg_0 ^ tag_reg_1 ^ tag_reg_2;

    // Permutation core
    ascon_permutation_d1 u_perm (
        .clk(clk),
        .reset(rst),
        .ctr(perm_ctr),
        ...
        if (rst) begin
            ...
            case (state)
                ...
                S_DONE: begin
                    ready <= 1'b1;
                    if (decrypt) begin
                        auth_valid <= ((tag_reg_0 ^ tag_reg_1 ^ tag_reg_2) == tag_in);
                    end else begin
                        auth_valid <= 1'b1;
                    end
                    if (!start)
